...
class Overwatch {
  ...
  private parseProfile(
    ...
    const level = $('.player-level div')
      .first()
      .text();
    const portrait = $('.player-portrait').attr('src');
    const permission = $('.masthead-permission-level-text').text();
    const endorsementLevel = $('.masthead .endorsement-level div')
      .last()
      .text();
    const endorsementFrameEl = $('.masthead .EndorsementIcon').attr('style');
    const sportsmanshipValue = $(
      '.masthead .EndorsementIcon-border--sportsmanship',
    ).data('value');
    const shotcallerValue = $(
      '.masthead .EndorsementIcon-border--shotcaller',
    ).data('value');
    const teammateValue = $('.masthead .EndorsementIcon-border--teammate').data(
      'value',
    );
    const quickplayWonEl = $('#quickplay td:contains("Games Won")')
      .next()
      .html();
    const quickplayTimePlayedEl = $('#quickplay td:contains("Time Played")')
      .next()
      .html();
    const compWonEl = $('#competitive td:contains("Games Won")')
      .next()
      .html();
    const compPlayedEl = $('#competitive td:contains("Games Played")')
      .next()
      .html();
    const compLostEl = $('#competitive td:contains("Games Lost")')
      .next()
      .html();
    const compDrawEl = $('#competitive td:contains("Games Tied")')
      .next()
      .html();
    const compTimePlayedEl = $('#competitive td:contains("Time Played")')
      .next()
      .html();
    ...
    const compRankImg =
      (compRankEl && $('.competitive-rank img').attr('src')) || null;
    const compRank = compRankEl && $('.competitive-rank div').html();
    const levelFrame =
      levelFrameEl && levelFrameEl.slice(21, -1).replace(/ /g, '');
    const endorsementFrame =
      endorsementFrameEl && endorsementFrameEl.slice(21, -1).replace(/ /g, '');
  ...
  private formatProfile(profileRaw: profileRaw): Profile {
    ...
    profile.endorsment.sportsmanship = parseFloat(
      profileRaw.sportsmanshipValue,
    );
    ...
    profile.quickplay.won =
      (profileRaw.quickplayWonEl &&
        parseInt(profileRaw.quickplayWonEl.trim().replace(/,/g, ''), 10)) ||
      0;
    profile.quickplay.time =
      profileRaw.quickplayTimePlayedEl &&
      moment.duration(profileRaw.quickplayTimePlayedEl);

    profile.competitive.won =
      (profileRaw.compWonEl &&
        parseInt(profileRaw.compWonEl.trim().replace(/,/g, ''), 10)) ||
      0;
    profile.competitive.lost =
      (profileRaw.compLostEl &&
        parseInt(profileRaw.compLostEl.trim().replace(/,/g, ''), 10)) ||
      0;
    profile.competitive.draw =
      (profileRaw.compDrawEl &&
        parseInt(profileRaw.compDrawEl.trim().replace(/,/g, ''), 10)) ||
      0;
    profile.competitive.played =
      (profileRaw.compPlayedEl &&
        parseInt(profileRaw.compPlayedEl.trim().replace(/,/g, ''), 10)) ||
      0;
    profile.competitive.time =
      profileRaw.compTimePlayedEl &&
      moment.duration(profileRaw.compTimePlayedEl);
    profile.competitive.rank =
      profileRaw.compRank && parseInt(profileRaw.compRank, 10);
    ...
    if (profileRaw.star && profileRaw.rank) {
      const starsMatch = path
        .basename(profileRaw.star)
        .split('.')
        .slice(0, -1)[0];
      const rankMatch = path
        .basename(profileRaw.rank)
        .split('.')
        .slice(0, -1)[0];
